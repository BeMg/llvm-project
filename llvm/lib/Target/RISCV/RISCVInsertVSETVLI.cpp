...
static void fixupModifyVRegLIFromMI(MachineInstr *MI, LiveIntervals *LIS) {
...
void RISCVInsertVSETVLI::insertVSETVLI(MachineBasicBlock &MBB,
  ...
  if (PrevInfo.isValid() && !PrevInfo.isUnknown()) {
    ...
    if (Info.hasSameAVL(PrevInfo) && Info.hasSameVLMAX(PrevInfo)) {
      ...
      fixupModifyVRegLIFromMI(NeedFixupMI, LIS);
      return;
    }
      ...
      if (RISCVII::hasVLOp(TSFlags)) {
        ...
        if (VLOp.isReg()) {
          ...
          if (VLOpDef && TII->isAddImmediate(*VLOpDef, Reg) &&
            ...
            SmallVector<Register> NeedFixupVReg;
            getVRegFromMI(VLOpDef, NeedFixupVReg);
            VLOpDef->eraseFromParent();
            if (LIS)
              LIS->RemoveMachineInstrFromMaps(*VLOpDef);
            for (auto FixReg : NeedFixupVReg)
              fixupModifyVRegLI(FixReg, LIS);
          }
          if (IsVirtVLOpReg)
